...
{
    ControllerMaster::getControllerMaster().start_controller(new p2pController(0.3, 0.4, goal_point), std::bind(&StateMachine::stop, &state_machine));
...
{
    ControllerMaster::getControllerMaster().start_controller(new p2pController(0.1, 0.1, goal_point), std::bind(&StateMachine::stop, &state_machine));
...
{   
    ControllerMaster::getControllerMaster().stop_controller();
    state_machine.setState(new Idle);
}