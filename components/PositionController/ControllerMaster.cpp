...
ControllerMaster::ControllerMaster() : _pos_controller{nullptr}, _output_velocity{OutputVelocity::getOutput()}, _sensor_pose{SensorPose::getSensor()}
{   
...
{   
    ...
    if(xSemaphoreTake(_pos_controller_mutx, portMAX_DELAY) == pdPASS)
    {
        ...
        _controller_obj->_callback_function = callback_function;
...
{   
    ...
    {   
        if(xSemaphoreTake(_pos_controller_mutx, 0) == pdPASS)
        {
            //Input
            ros_msgs::Pose2D actual_pose = _controller_obj->_sensor_pose.get_Pose();
            ...
            if(_controller_obj->_pos_controller->destination_reached() == true)
                _controller_obj->_callback_function();
